...
exports.ingestSubscriptions = ingestSubscriptions;
...
async function ingestSubscriptions() {
    ...
    for (const adapter of adapters) {
        ...
        try {
            ...
            logger_1.logger.error("Subscription adapter fetch failed", { error: message });
            errors.push(message);
            continue;
        }
        for (const record of records) {
            ...
            if (!ipo || (ipo.status !== "OPEN" && ipo.status !== "CLOSING_TODAY"))
                continue;
