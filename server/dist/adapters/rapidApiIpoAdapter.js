...
function logRawSampleOnce(alreadyLogged, label, data) {
    if (alreadyLogged)
        return;
    logger_1.logger.info(`RapidAPI raw sample response for ${label} (first call only)`, {
...
async function toRapidApiError(res) {
    const body = await res.text().catch(() => "");
    const snippet = body.slice(0, 300);
    switch (res.status) {
        case 401:
            return new RapidApiAuthError(`RapidAPI 401 Unauthorized — RAPIDAPI_KEY is missing/invalid. Check server/.env. (${snippet})`);
        case 403:
            return new RapidApiAuthError(`RapidAPI 403 Forbidden — key is valid but not subscribed to "${env_1.env.rapidApi.host}" (or plan/quota doesn't allow this endpoint). (${snippet})`);
        case 429:
            return new Error(`RapidAPI 429 Too Many Requests — rate limit hit for "${env_1.env.rapidApi.host}". (${snippet})`);
        case 500:
        case 502:
        case 503:
        case 504:
...
async function rapidApiGet(path) {
    ...
    try {
        return await (0, retry_1.withRetry)(async () => {
            const res = await fetch(`${BASE_URL}${path}`, {
                headers: {
                    ...
                    "x-rapidapi-host": env_1.env.rapidApi.host,
                    "x-rapidapi-key": env_1.env.rapidApi.key,
                },
            });
            if (!res.ok) {
                const error = await toRapidApiError(res);
                // 401/403 won't be fixed by retrying with the same bad key — fail fast.
                if (error instanceof RapidApiAuthError)
                    throw new retry_1.NoRetryError(error.message);
                throw error;
            }
        ...
        if (err instanceof retry_1.NoRetryError)
...
function cleanString(value) {
    ...
    if (!trimmed || trimmed.toLowerCase() === "data not found")
        return undefined;
    return trimmed;
}
...
function normalizeCurrentIpo(item) {
    ...
    if (!rawName || !openDate || !closeDate || !priceRange)
        return null;
...
class RapidApiIpoAdapter {
    async fetchUpcomingIPOs() {
        ...
        const errors = [];
        try {
            const data = await rapidApiGet("/main_ipo_public?limit=50&offset=0");
            ...
            logger_1.logger.info(`RapidAPI /main_ipo_public: parsed ${records.length}/${currentSeen} complete records (rest lacked name/dates/price)`);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            logger_1.logger.error("RapidAPI /main_ipo_public failed", { error: message });
            errors.push(message);
        }
        const beforeListed = records.length;
        try {
            const data = await rapidApiGet("/listed_main_ipo_public?order=id.desc&limit=50&offset=0");
            ...
            logger_1.logger.info(`RapidAPI /listed_main_ipo_public: parsed ${records.length - beforeListed}/${listedSeen} records`);
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            logger_1.logger.error("RapidAPI /listed_main_ipo_public failed", { error: message });
            errors.push(message);
        }
        if (errors.length === 2) {
    ...
    async fetchGMP() {
        logger_1.logger.warn("Indian IPO Wallah has no GMP field — fetchGMP() intentionally returns no records. Configure IPOGURU_API_KEY or another GMP source instead.");
        return [];
    }
...
exports.RapidApiIpoAdapter = RapidApiIpoAdapter;
exports.rapidApiIpoAdapter = new RapidApiIpoAdapter();
